...
func TestXormAliDb(t *testing.T) {
	url := "root:Makeblock123@tcp(rm-wz912s848qk08h72c.mysql.rds.aliyuncs.com:3306)/content-platform?charset=utf8"
	...
	println(engine)
	//creatTable(engine)
	results, err := engine.QueryString("select * from cp_users limit 10")
	if err != nil {
		fmt.Println(err)
	}
